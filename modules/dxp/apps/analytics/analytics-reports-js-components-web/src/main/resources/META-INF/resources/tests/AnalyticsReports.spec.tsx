...
describe('AnalyticsReports Overview Metrics', () => {
	...
	it('is able to see 4 metrics on Overview Metric component: Views, Reading Time, Comments and Rating', async () => {
		mockedFetch.mockReturnValueOnce(
			Promise.resolve(new Response(JSON.stringify(MOCKED_CONNECTED_DATA)))
		);
	...
	it('is able to see 4 metrics on Overview Metric component: Downloads, Previews and Comments', async () => {
		mockedFetch.mockReturnValueOnce(
			Promise.resolve(new Response(JSON.stringify(MOCKED_CONNECTED_DATA)))
		);
