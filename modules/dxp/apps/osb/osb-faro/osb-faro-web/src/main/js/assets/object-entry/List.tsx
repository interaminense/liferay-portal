...
const List = () => {
	...
	useEffect(() => {
		async function fetchFDS() {
			const all = await import(
				// @ts-ignore
				'http://localhost:3000/o/frontend-data-set-web/__liferay__/index.js'
			);
	...
	// console.log({test: FDSComponent?.FrontendDataSet});

	const Component = FDSComponent?.FrontendDataSet;
	...
	return (
		<BasePage documentTitle={Liferay.Language.get('assets')}>
			...
			<BasePage.Body>
				<Card>
					{Component && <Component {...props} views={[tableView]} />}
				</Card>
			</BasePage.Body>
		</BasePage>
	);
