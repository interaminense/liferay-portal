...
const layoutReducer = (state: TState, action: TAction) => {
	switch (action.type) {
		...
		case TYPES.ADD_NEW_BLOCK: {
			const {label, partitionNumbers, tabIndex} = action.payload;
		...
		case TYPES.ADD_NEW_FIELD: {
			const {boxIndex, label, tabIndex} = action.payload;

			const newState = {...state};
			const newField = {label, readOnly: false, type: 'Text'};
		...
		case TYPES.DELETE_FIELD: {
			...
			newState.layoutData.tabs[tabIndex].boxs[boxIndex].rows.splice(rowIndex, 1);
...
export const LayoutContextProvider: React.FC<React.HTMLAttributes<HTMLElement>> = ({children}) => {
