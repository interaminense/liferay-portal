 ...
 */
class Analytics {
	[AnalyticsType.StorageKeys.Events]: EventQueue | null = null;
	[AnalyticsType.StorageKeys.Messages]: EventMessageQueue | null = null;
	[AnalyticsType.StorageKeys.IdentityMessage]: IdentityMessageQueue | null =
		null;
	...
	getEvents() {
		...
		return this[
			AnalyticsType.StorageKeys.Events
	...
	reset() {
		...
		this[AnalyticsType.StorageKeys.Events].reset();
	...
	track(
		...
		this[AnalyticsType.StorageKeys.Events]?.addItem(
			normalizeEvent(
				eventId,
	...
	_ensureIntegrity() {
		const userId = getItem<string>(AnalyticsType.StorageKeys.UserId);

		if (userId) {
			this._setCookie(AnalyticsType.StorageKeys.UserId, userId);
		}
	}
	 ...
	 */
	_getUserId() {
		let userId = getItem<string>(AnalyticsType.StorageKeys.UserId);

		const {emailAddressHashed} = this.config.identity;
		const previousEmailAddressHashed = getItem<string>(
			AnalyticsType.StorageKeys.PrevEmailAddressHash
		);
	...
	_generateUserId() {
		...
		setItem(AnalyticsType.StorageKeys.UserId, userId);
		this._setCookie(AnalyticsType.StorageKeys.UserId, userId);

		removeItem(AnalyticsType.StorageKeys.Identity);
	...
	_sendIdentity(identity: AnalyticsType.Config['identity'], userId: string) {
		...
		const storedIdentityHash = getItem<string>(
			AnalyticsType.StorageKeys.Identity
		);
		const storedChannelId = getItem<string>(
			AnalyticsType.StorageKeys.ChannelId
		);
		...
		if (
			...
			setItem(AnalyticsType.StorageKeys.ChannelId, channelId);
			setItem(AnalyticsType.StorageKeys.Identity, identityHash);

			this[AnalyticsType.StorageKeys.IdentityMessage]?.addItem({
				channelId,
				dataSourceId,
				emailAddressHashed,
				id: identityHash,
	...
	_initializeEventQueue() {
		...
		this[AnalyticsType.StorageKeys.Events] = eventQueue;
	...
	_initializeEventMessageQueue() {
		...
		this[AnalyticsType.StorageKeys.Messages] = eventMessageQueue;
	...
	_initializeIdentityMessageQueue() {
		...
		this[AnalyticsType.StorageKeys.IdentityMessage] = identityMessageQueue;
...
// Exposes AnalyticsType.create to the global scope
