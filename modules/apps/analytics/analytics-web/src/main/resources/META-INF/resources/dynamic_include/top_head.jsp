...
<aui:script id="liferayAnalyticsScript" senna="permanent" type="text/javascript">
	function <portlet:namespace />checkLiferayDXPConsent() {
	...
	function <portlet:namespace />validateConsentCookie() {
		var consentCookies = [<portlet:namespace />checkLiferayDXPConsent()];

		return [checkLiferayDXPConsent()].some((hasConsent) => hasConsent);
	}
			...
			<c:if test="<%= GetterUtil.getBoolean(PropsUtil.get(PropsKeys.JAVASCRIPT_SINGLE_PAGE_APPLICATION_ENABLED)) %>">
				...
				if (Liferay.FeatureFlags['LPD-10588']) {
					Liferay.on('cookieBannerSetCookie', () => {
						if (<portlet:namespace />validateConsentCookie()) {
