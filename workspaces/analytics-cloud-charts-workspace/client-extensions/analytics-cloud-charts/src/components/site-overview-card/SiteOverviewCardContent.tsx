...
import {Metric, SiteMetric} from '../../lib/analytics';
...
export interface SiteOverviewCardContentIProps {
	metric: SiteMetric | null;
}

interface KpiDefinition {
	format: KpiCardFormat;
	getMetric: (metric: SiteMetric) => Metric | null;
	label: string;
}
...
const KPI_DEFINITIONS: KpiDefinition[] = [
	...
	{
		format: 'number',
		getMetric: (metric) => metric.sessionsMetric,
		label: 'Sessions',
	},
	{
		format: 'duration',
		getMetric: (metric) => metric.sessionDurationMetric,
...
export const SiteOverviewCardContent: React.FC<
	SiteOverviewCardContentIProps
> = ({metric}) => {
	...
	if (!metric || !hasAnyData) {
		return (
			<ClayEmptyState
				description="There are no site KPIs available for the selected period."
...
export default SiteOverviewCardContent;
