/**
 * SPDX-FileCopyrightText: (c) 2000 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

const DocumentsAndMediaListResolver = function DocumentsAndMediaListResolver() {
	return {
		__typename: 'AssetMetricBag',

		assetMetrics: [
			{
				__typename: 'DocumentMetric',
				assetId: '276627612',
				assetTitle:
					'Como Construir uma Intranet que Impulsione a Produtividade',
				commentsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				downloadsMetric: {
					__typename: 'Metric',
					value: 4,
				},
				impressionMadeMetric: {
					__typename: 'Metric',
					value: 13,
				},
				ratingsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				urls: null,
			},
			{
				__typename: 'DocumentMetric',
				assetId: '25842680',
				assetTitle: 'Deploying Liferay DXP in Azure',
				commentsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				downloadsMetric: {
					__typename: 'Metric',
					value: 3,
				},
				impressionMadeMetric: {
					__typename: 'Metric',
					value: 13,
				},
				ratingsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				urls: null,
			},
			{
				__typename: 'DocumentMetric',
				assetId: '441038489',
				assetTitle: 'Liferay DXP 7.4 Deployment Checklist',
				commentsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				downloadsMetric: {
					__typename: 'Metric',
					value: 3,
				},
				impressionMadeMetric: {
					__typename: 'Metric',
					value: 5,
				},
				ratingsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				urls: null,
			},
			{
				__typename: 'DocumentMetric',
				assetId: '454879125',
				assetTitle: 'Upgrading from Liferay 7.x to 7.4',
				commentsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				downloadsMetric: {
					__typename: 'Metric',
					value: 2,
				},
				impressionMadeMetric: {
					__typename: 'Metric',
					value: 24,
				},
				ratingsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				urls: null,
			},
			{
				__typename: 'DocumentMetric',
				assetId: '391072194',
				assetTitle:
					'5 Ways Self-Service is Valuable for Wealth Managers',
				commentsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				downloadsMetric: {
					__typename: 'Metric',
					value: 2,
				},
				impressionMadeMetric: {
					__typename: 'Metric',
					value: 3,
				},
				ratingsMetric: {
					__typename: 'Metric',
					value: 0,
				},
				urls: null,
			},
		],

		total: 5,
	};
};

/**
 * SPDX-FileCopyrightText: (c) 2000 Liferay, Inc. https://liferay.com
 * SPDX-License-Identifier: LGPL-2.1-or-later OR LicenseRef-Liferay-DXP-EULA-2.0.0-2023-06
 */

export default DocumentsAndMediaListResolver;
