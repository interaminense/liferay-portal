...
describe('BaseResults', () => {
	...
	it('renders w/a no results display', async () => {
		const {getByText} = render(
			<DefaultComponent
	...
	it('puts a size limit on the query based on maxLength', () => {
		const {container} = render(
			<DefaultComponent
				dataSourceFn={() =>
					Promise.resolve({items: INDIVIDUALS, total: TOTAL})
	...
	it('does not render a subnav if there is a datasourceFn error', async () => {
	...
	it('renders with search disabled when disableSearch is TRUE', async () => {
		const {container} = render(
			<DefaultComponent
				dataSourceFn={() =>
					Promise.resolve({
	...
	it('does not include disabled items when calculating whether all the items are checked', async () => {
	...
	it('does not re-invoke resultsRenderer while typing in the search input', async () => {
